...
function getCurrencySymbol()
{
    ...
    return collect($currenciesData)->where('currency_code',
...
function redirectToDashboard(): string
{
    ...
    if ($user->hasRole('Super Admin')) {
        ...
        if ($user->hasRole('Admin')) {
            ...
            $module = Module::whereNotIn('name',
                Module::Doctor)->whereIsActive(1)->whereTenantId(getLoggedInUser()->tenant_id)->first();
            if ($module) {
                return route($module->route);
            }
            ...
            $module = Module::whereNotIn('name',
                Module::Pharmacist)->whereIsActive(1)->whereTenantId(getLoggedInUser()->tenant_id)->first();
            if ($module) {
                return route($module->route);
            }
            ...
            $module = Module::whereNotIn('name',
                Module::LAB_TECHNICIAN)->whereIsActive(1)->whereTenantId(getLoggedInUser()->tenant_id)->first();
            if ($module) {
                return route($module->route);
            }
            ...
            $module = Module::whereNotIn('name',
                Module::CASE_HANDLER)->whereIsActive(1)->whereTenantId(getLoggedInUser()->tenant_id)->first();
            if ($module) {
                return route($module->route);
            }
            ...
            $module = Module::whereNotIn('name',
                Module::Patient)->whereIsActive(1)->whereTenantId(getLoggedInUser()->tenant_id)->first();
            if ($module) {
                return route($module->route);
            }
            ...
            $module = Module::whereNotIn('name',
                Module::Nurse)->whereIsActive(1)->whereTenantId(getLoggedInUser()->tenant_id)->first();
            if ($module) {
                return route($module->route);
            }
            ...
            $module = Module::whereNotIn('name',
                Module::Accountant)->whereIsActive(1)->whereTenantId(getLoggedInUser()->tenant_id)->first();
            if ($module) {
                return route($module->route);
            }
...
function getMenuLinks($menu)
{
    ...
    //ER
    if ($menu == User::MAIN_ER) {
        $defaultRoute = route('er.patient.index');
        $subMenus = ['ER'];
        // dd($subMenus);
...
/**
 * @param $routeName
 * @param $subscriptionPlanFeaturesId
 * @return bool
 */
